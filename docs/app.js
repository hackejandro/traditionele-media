...
function openLink(url,scroll=true,updateHistory=true,trackOpen=true){
  ...
  if(trackOpen)window.traditioneleMediaAnalytics?.track('conversation_open',{conversationId:item.id||'',conversationUrl:item.url,conversationTitle:item.title||fallbackTitle(item.url)});
